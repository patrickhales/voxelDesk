...
class DICOMSorter(object):
    ...
    def setOptions(self,options):
        ...
        if '%' not in options['targetPattern']:
            ...
            pattern = '/%PatientName/%StudyDate/%SeriesNumber-%SeriesDescription/%SeriesDescription-%InstanceNumber.dcm'
    ...
    def pathFromDatasetPattern(self,ds,safe=True):
        ...
        for key in keys:
            if hasattr(ds,key):
                ...
                # added by PH to add leading zeros
                if key == 'SeriesNumber' or key == 'InstanceNumber':
                    if int(value) < 10:
                        replacements[key] = '00' + str(value)
                        value = replacements[key]
                    if int(value) >= 10 and int(value) < 100:
                        replacements[key] = '0' + str(value)
                        value = replacements[key]
            else:
                value = ""
            if value == "":
    ...
    def renameFiles(self):
        ...
        if self.options['sourceDir'] == "":
            ...
            for root, subFolders, files in os.walk(self.options['sourceDir']):
                for file in files:
                    ...
                    """
                    # PH: check to see if this is a pre-sorted file - if so, exclude it. An exception is when we are
                    # running the sorter after a previous anonymization
                    if '_anon' not in self.options['sourceDir']:
                        if self.config['sortedDicomFolderName'] not in file:
        ...
        if self.options['verbose']:
            print("Renamed %d, skipped %d files:" % (self.filesRenamed, self.filesSkipped))
    ...
    def renameFile(self,file):
        ...
        try:
            ...
            # adapted by PH to add decompression as standard
            if ds['PixelData'].is_undefined_length:
                ...
                # pydicom will set the is_undefined_length flag automatically to match the transfer syntax when writing to file.
                """
